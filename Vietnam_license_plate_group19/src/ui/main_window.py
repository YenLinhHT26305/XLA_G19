...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.camera_url = "http://192.168.1.213:81/stream"
    ...
    # ================= GIAO DIỆN THEO PHONG CÁCH ẢNH BẠN GỬI =================
    def build_ui(self):
        # 1. Thiết lập nền tối (Dark Theme) toàn ứng dụng
        self.setStyleSheet("""
        ...
        # Viền trắng mỏng, nền đen như trong ảnh
        ...
        # --- CÁC NHÃN THÔNG TIN (Bên phải dưới) ---
        # Font chữ trắng, to rõ
        ...
        # --- CÁC NÚT BẤM (Bên dưới Camera) ---
        # Văn phong nút y hệt ảnh: Icon + Text
        ...
        # Style nút: Nền xám đậm, chữ trắng, bo góc nhẹ (giống ảnh)
        btn_style = """
            QPushButton {
        ...
        left_layout.addLayout(btn_layout) # Nút nằm ngay dưới camera

        # Cột phải: Ảnh Crop + Thông tin
    ...
    def make_label(self, text, size, bold=False):
        ...
        # Style chữ trắng trên nền tối
        lbl.setStyleSheet(f"font-size: {size}px; font-weight: {weight}; color: white; padding: 2px;")
        lbl.setWordWrap(True)
        return lbl

    # ================= LOGIC GIỮ NGUYÊN (UPDATED) =================
    def start_camera(self):
        # 1. Kết nối lại Arduino
        self.barrier.connect_arduino()

        # 2. Mở Camera
        self.cap = cv2.VideoCapture(self.camera_url)
        if not self.cap.isOpened():
             QMessageBox.warning(self, "Lỗi", "Không kết nối được Camera Wifi!")
             return
        self.timer.start(30)
        ...
        if self.frame_count % 5 == 0:
            ...
            if crop is not None:
                ...
                if plate:
                    ...
                    # TRẠNG THÁI (Đổi màu chữ để nổi bật trên nền đen)
                    if allowed:
                        self.lbl_status.setText("Trạng thái: ✅ ĐƯỢC PHÉP")
                        self.lbl_status.setStyleSheet("color: #00FF00; font-weight: bold; font-size: 22px;") # Xanh lá sáng
                        self.is_allowed_signal = True 
                    else:
                        self.lbl_status.setText("Trạng thái: ❌ KHÔNG ĐƯỢC PHÉP")
                        self.lbl_status.setStyleSheet("color: #FF3333; font-weight: bold; font-size: 22px;") # Đỏ sáng
                ...
                if self.no_plate_count > 40 and "Đang chờ" not in self.lbl_status.text():
                    self.reset_ui()
    ...
    def reset_ui(self):
        ...
        self.lbl_status.setStyleSheet("color: #FFFF00; font-weight: bold; font-size: 22px;") # Vàng sáng
    ...
    def closeEvent(self, event):
        self.stop_camera()
        if hasattr(self, 'barrier') and self.barrier: # Kiểm tra kĩ trước khi đóng
            self.barrier.close_connection()
        event.accept()